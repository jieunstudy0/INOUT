...
class DepositLedgerConcurrencyTest {
    ...
    @Autowired private OrderOwnerService orderOwnerService;
    ...
    void setUp() {
        ...
        tx.executeWithoutResult(s -> {
            ...
            owner = userRepository.save(User.builder()
                    .email("owner-" + suffix + "@inout-test.com")
                    .password("encoded")
                    .name("점주-" + suffix)
                    .phone("010-8888-" + suffix.substring(0, 4))
    ...
    @Test
    @DisplayName("[동시성] 100건 동시 점주 승인·결제: DepositHistory 100건 누락 없이 기록되고 balanceAfter가 정확하다")
    void concurrentPayment_100Payments_ledgerIsCompleteAndAccurate() throws Exception {

        AtomicInteger successCount = new AtomicInteger();
        runConcurrently(WORKER_COUNT, i -> {
            orderOwnerService.modifyAndApprove(
                    owner.getId(),
                    orders.get(i).getId(),
                    OwnerOrderModifyRequest.builder()
                            .items(List.of(OwnerOrderModifyRequest.ItemLine.builder()
        ...
        tx.executeWithoutResult(s -> {
            ...
            assertThat(reloaded)
                    .as("결제 후 모든 주문은 ORDERED 상태여야 한다")
                    .extracting(OrderRequest::getStatus)
                    .containsOnly(OrderStatus.ORDERED);
        });
    ...
    void concurrentApproval_100Orders_stockAndLedgerConsistent() throws Exception {
        ...
        tx.executeWithoutResult(s -> {
            ...
            assertThat(reloaded)
                    ...
                    .extracting(OrderRequest::getStatus)
                    .containsOnly(OrderStatus.APPROVED);
        });
