...
public class OrderPreResponse {
    ...
    public static OrderPreResponse from(User user, List<CartDetail> cartDetails) {
        String address = user.getStore() != null ? user.getStore().getAddress() : "";
        return OrderPreResponse.builder()
                .storeName(UserDisplayNames.storeName(user))
                .employeeName(UserDisplayNames.displayName(user))
                .storeAddress(address != null ? address : "")
