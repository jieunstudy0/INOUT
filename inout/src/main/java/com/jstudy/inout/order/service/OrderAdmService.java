...
public class OrderAdmService {
    ...
    public List<OrderAdminResponse> getAllOrders(OrderStatus status) {
        List<OrderRequest> orders = (status == null) 
            ? orderRequestRepository.findAllWithDetailsOrderByDateDesc() 
            : orderRequestRepository.findAllWithDetailsByStatusOrderByDateDesc(status);
