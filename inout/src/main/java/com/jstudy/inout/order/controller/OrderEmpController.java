...
public class OrderEmpController {
    ...
    @Operation(summary = "발주 신청",
               description = """
                       장바구니에서 선택한 품목으로 발주를 생성합니다.
                       - 발주 상태: **REQUESTED** (결제 대기)
                       - 수신자·주소 미입력 시 직원 기본 정보로 자동 채워집니다.
                       - 성공 시 해당 장바구니 항목은 논리 삭제됩니다.
                       """)
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "발주 신청 성공 (주문 ID 반환)"),
    ...
    public ResponseEntity<?> submitOrder(
        ...
        return ResponseResult.success("주문서가 생성되었습니다.", orderId);
    }

    @Operation(summary = "내 발주 이력 목록 조회",
               description = "로그인한 직원의 결제 완료된 발주 목록을 최신순으로 반환합니다.")
