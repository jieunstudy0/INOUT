...
public class AuthServiceImpl implements AuthService {
    ...
    public AdminUserDto.ListResponse getAdminUserList(Long storeId, String status, String keyword, Pageable pageable) {
        ...
        long leave = userRepository.countByStatus(UserStatus.ON_LEAVE);
        ...
        if (StringUtils.hasText(status)) {
            try {
                userStatus = UserStatus.fromApi(status);
        ...
        Page<AdminUserDto.UserListItem> userItems = userPage.map(u -> {
            ...
            return AdminUserDto.UserListItem.builder()
                    ...
                    .storeName(u.getStore() != null ? u.getStore().getName() : "본점 (소속 없음)")
                    .status(u.getStatus())
                    .isLocked(u.isLocked())
                    .isAdmin(isAdmin)
    ...
    public OwnerUserDto.ListResponse getOwnerUserList(Long ownerUserId, String status, String keyword, Pageable pageable) {
        ...
        long total = userRepository.countByStore_IdAndStatusNot(owner.getStore().getId(), UserStatus.RESIGNED);
        long active = userPage.getContent().stream().filter(u -> u.getStatus() == UserStatus.ACTIVE).count();
        long leave = userPage.getContent().stream().filter(u -> u.getStatus() == UserStatus.ON_LEAVE).count();
        long resigned = userPage.getContent().stream().filter(u -> u.getStatus() == UserStatus.RESIGNED).count();
        long locked = userRepository.countByStore_IdAndIsLockedTrueAndStatusNot(
                owner.getStore().getId(), UserStatus.RESIGNED);
        ...
        Page<OwnerUserDto.UserListItem> users = userPage.map(u -> OwnerUserDto.UserListItem.builder()
                ...
                .createdAt(u.getCreatedAt())
                .dailyDepositLimit(u.getDailyDepositLimit())
                .todayUsedDeposit(u.getTodayUsedDeposit() != null ? u.getTodayUsedDeposit() : 0L)
                .build());
    ...
    public void updateEmployeeByOwner(Long ownerUserId, Long employeeUserId, OwnerUserDto.UpdateRequest request) {
        if (request == null || request.getStatus() == null) {
            throw new InoutException("상태(status)는 필수입니다.", 400, "STATUS_REQUIRED");
        }
        User owner = requireOwnerWithStore(ownerUserId);
    ...
    @Override
    @Transactional
    public void updateEmployeeDepositLimitByOwner(
            Long ownerUserId, Long employeeUserId, OwnerUserDto.DepositLimitRequest request) {
        User owner = requireOwnerWithStore(ownerUserId);
        User employee = requireSameStoreEmployee(owner, employeeUserId);
        try {
            employee.updateDailyDepositLimit(request != null ? request.getDailyDepositLimit() : null);
        } catch (IllegalArgumentException e) {
            throw new InoutException(e.getMessage(), 400, "INVALID_DAILY_DEPOSIT_LIMIT");
        }
    }
    ...
    private UserStatus parseUserStatus(String status) {
        ...
        try {
            return UserStatus.fromApi(status);
        } catch (IllegalArgumentException e) {
            return null;
        }
    }
