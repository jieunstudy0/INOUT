...
function OrderTable({ orders, checkedIds, onCheckAll, onCheck, onApprove, onReject, actionBusyId }) {
  ...
  const isAwaitingHq = (status) => status === 'ORDERED' || status === 'PAID';
        ...
        <table className="min-w-full divide-y divide-slate-200">
          ...
          <tbody className="divide-y divide-slate-100 bg-white">
            {orders.map((order) => {
              ...
              const busy = actionBusyId === order.orderRequestId;
              return (
                <tr key={order.orderRequestId}
                  ...
                  <td className="px-4 py-3.5">
                    <p className="text-sm font-medium text-slate-800"><PersonName name={order.employeeName || '-'} /></p>
                    <p className="text-xs text-slate-400 mt-0.5">{order.storeName || '본점 (소속 없음)'}</p>
                  </td>
                  <td className="px-4 py-3.5">
                      ...
                      {isAwaitingHq(order.status) && (
                        <>
                          <button
                            type="button"
                            disabled={busy}
                            onClick={() => onApprove(order.orderRequestId)}
                            className="px-2.5 py-1.5 text-[11px] font-semibold rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-60"
                          >
                            최종 승인
                          </button>
                          <button
                            type="button"
                            disabled={busy}
                            onClick={() => onReject(order.orderRequestId)}
                            className="px-2.5 py-1.5 text-[11px] font-semibold rounded-lg bg-rose-50 text-rose-700 border border-rose-200 hover:bg-rose-100 disabled:opacity-60"
                          >
                            발주 반려
                          </button>
                        </>
...
export default function OrderAdmPage() {
  ...
  const handleBulkApprove = async () => {
    ...
    const paidOrders = orders.filter((o) => checkedIds.has(o.orderRequestId) && (o.status === 'ORDERED' || o.status === 'PAID'));
    if (paidOrders.length === 0) {
      Toast.warning('본사 승인 대기(ORDERED) 상태의 발주만 일괄 승인할 수 있습니다.');
      return;
    }
  ...
  const handleApproveOne = async (orderId) => {
    ...
    try {
      ...
      Toast.success(`발주 #${orderId} 최종 승인 완료`);
      loadOrders(activeTab);
      loadSummary();
    } catch { /* interceptor */ }
  ...
  const handleRejectOne = async (orderId) => {
    if (!window.confirm(`발주 #${orderId}을(를) 반려하고 예치금을 환불할까요?`)) return;
    setActionBusyId(orderId);
    try {
      await rejectOrder(orderId, { reason: '본사 발주 반려' });
      Toast.success(`발주 #${orderId} 반려 및 예치금 환불 완료`);
      loadOrders(activeTab);
      loadSummary();
    } catch { /* interceptor */ }
    finally { setActionBusyId(null); }
  ...
  const checkedPaidCount = orders.filter((o) => checkedIds.has(o.orderRequestId) && (o.status === 'ORDERED' || o.status === 'PAID')).length;
    ...
    <div className="space-y-5 max-w-7xl mx-auto">
      ...
      {!loading && orders.length > 0 && (
        <OrderTable orders={orders} checkedIds={checkedIds}
          ...
          onApprove={handleApproveOne} onReject={handleRejectOne}
