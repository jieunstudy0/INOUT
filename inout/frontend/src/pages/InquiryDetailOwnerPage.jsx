...
export default function InquiryDetailOwnerPage() {
  ...
  const handleDeleteInquiry = async () => {
    ...
    try {
      ...
      navigate('/owner/inquiries', { replace: true });
    } catch {}
  };
  ...
  const handleFileDownload = async () => {
    ...
    } catch { /* interceptor */ } finally { setDownloading(false); }
  };
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      await createComment(inquiryId, { content: commentContent.trim(), parentId: replyTo ? replyTo.id : null });
      setCommentContent('');
      setReplyTo(null);
      loadDetail();
    } catch {}
  ...
  const handleEditStart = (comment) => { setEditingCommentId(comment.id); setEditContent(comment.content); setReplyTo(null); };
  const handleEditCancel = () => { setEditingCommentId(null); setEditContent(''); };
  ...
  const isDeletedComment = (c) => c.content === '삭제된 댓글입니다.' || c.isDeleted;
  const fileName    = detail.originalFileName || detail.fileName || detail.attachmentName;
  const hasAttachment = Boolean(fileName && (detail.savedFilePath || detail.filePath || detail.fileUrl || detail.originalFileName));

  const isAuthor = currentUser && (
    String(currentUser.id) === String(detail.authorId) ||
    String(currentUser.userId) === String(detail.authorId)
  );
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">문의 내용과 답변을 확인합니다.</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => navigate('/owner/inquiries')} className="px-4 py-2 bg-white border border-slate-300 text-slate-600 text-sm font-bold rounded-xl hover:bg-slate-50">
            목록으로
          </button>
          {isAuthor && (
            <button onClick={handleDeleteInquiry} className="px-4 py-2 bg-rose-50 text-rose-600 text-sm font-bold rounded-xl hover:bg-rose-100">
              글 삭제
            </button>
          )}
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-8 py-6 border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-2 mb-2">
            <TargetTypeBadge targetType={detail.targetType} />
          </div>
        ...
        {hasAttachment && (
          <div className="px-8 py-4 bg-slate-50/90 border-t border-slate-100 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-10 h-10 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center font-bold text-lg shrink-0">📎</div>
            ...
            <button type="button" onClick={handleFileDownload} disabled={downloading} className="px-4 py-2 bg-indigo-600 text-white text-xs font-bold rounded-xl hover:bg-indigo-700 transition-colors shadow-sm disabled:opacity-50 whitespace-nowrap">
                    ...
                    <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                      <div className="flex justify-between items-center mb-2">
                        <div className="flex items-center gap-2 text-sm">
                          <span className={`font-bold ${isMine ? 'text-slate-800' : 'text-indigo-600'}`}><PersonName name={comment.authorName} /></span>
                        ...
                        {!isDeletedComment(comment) && (
                          <div className="flex items-center gap-3">
                            <button onClick={() => { setReplyTo({ id: comment.id, name: comment.authorName }); handleEditCancel(); }} className="text-xs text-slate-500 hover:text-slate-800 font-medium">답글</button>
                            {isMine && (
                              <>
                      ...
                      {editingCommentId === comment.id ? (
                        <div className="mt-3">
                          <textarea value={editContent} onChange={(e) => setEditContent(e.target.value)} className="w-full px-4 py-3 border border-slate-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-slate-500" rows="2" />
                        ...
                        <p className={`text-sm whitespace-pre-wrap leading-relaxed ${isDeletedComment(comment) ? 'text-slate-400 italic' : 'text-slate-600'}`}>{comment.content}</p>
                      )}
                    </div>

                    {childComments.filter((r) => r.parentId === comment.id).map((reply, ci) => {
                      const childKey = reply.id ? `child-${reply.id}` : `child-idx-${ci}`;
                      ...
                      return (
                        <div key={childKey} className="ml-10 relative">
                          ...
                          <div className="bg-slate-50/70 p-4 rounded-xl border border-slate-200 shadow-sm">
                            <div className="flex justify-between items-center mb-2">
                              <div className="flex items-center gap-2 text-sm">
                                <span className={`font-bold ${isChildMine ? 'text-slate-800' : 'text-indigo-600'}`}><PersonName name={reply.authorName} /></span>
                            ...
                            {editingCommentId === reply.id ? (
                              ...
                              <p className={`text-sm whitespace-pre-wrap leading-relaxed ${isDeletedComment(reply) ? 'text-slate-400 italic' : 'text-slate-600'}`}>{reply.content}</p>
                            )}
                          </div>
                        </div>
                      );
          ...
          <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
            {replyTo && (
              <div className="flex items-center justify-between bg-slate-100 px-4 py-2 rounded-lg mb-3 border border-slate-200">
                <span className="text-xs font-semibold text-slate-700"><span className="font-bold">@{replyTo.name}</span> 님에게 답글 작성 중...</span>
