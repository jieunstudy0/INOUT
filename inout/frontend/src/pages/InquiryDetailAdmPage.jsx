...
import { triggerAiCsClassificationForOne } from '../api/aiApi';
...
export default function InquiryDetailAdmPage() {
  ...
  const handleAiDraft = async () => {
    setAiRunning(true);
    try {
      const data = await triggerAiCsClassificationForOne(inquiryId);
      Toast.success(data?.message || 'AI 초안 생성이 완료되었습니다.');
      loadDetail();
    } catch {
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-8 py-6 border-b border-slate-100 bg-slate-50/50">
          ...
          <div className="flex items-center gap-3 text-sm text-slate-500">
            <span className="font-semibold text-slate-700"><PersonName name={detail.authorName} /></span>
            <span>|</span>
                      ...
                      <div className="flex justify-between items-center mb-3">
                        <div className="flex items-center gap-2 text-sm">
                          <span className={`font-bold ${isMine ? 'text-indigo-600' : 'text-slate-800'}`}>
                            <PersonName name={comment.authorName} /> {isMine && '(본사)'}
                          </span>
                              ...
                              <div className="flex justify-between items-center mb-2">
                                <div className="flex items-center gap-2 text-sm">
                                  <span className={`font-bold ${isChildMine ? 'text-indigo-600' : 'text-slate-800'}`}>
                                    <PersonName name={reply.authorName} /> {isChildMine && '(본사)'}
                                  </span>
