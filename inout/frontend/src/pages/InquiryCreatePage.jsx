...
export default function InquiryCreatePage() {
  ...
  const backPath = isOwner ? '/owner/inquiries' : '/emp/inquiries';
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">문의 내용을 상세히 적어주세요. 필요시 파일을 첨부할 수 있습니다.</p>
      </div>

      {/* AI 안내 배너 (본사 문의인 경우에만) */}
      {targetType === 'ADMIN' && (
        <div className="rounded-2xl border border-violet-200 bg-violet-50/60 px-5 py-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-violet-600/90 mt-1 leading-relaxed">
              문의 등록 후 Gemini가 내용을 분석해 카테고리와 답변 초안을 생성합니다.
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-3">
              수신 대상 <span className="text-rose-500">*</span>
            </label>
            <div className="flex gap-3">
              <label className={`flex-1 flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${
                ...
                />
                <div>
                  <p className="text-sm font-bold text-slate-800">🏢 본사 (HQ)</p>
                  <p className="text-xs text-slate-500 mt-0.5">발주, 시스템, 정책 등 본사 관련 문의</p>
                </div>
              </label>

              {/* 직원만 점주에게 문의 가능 (점주는 본사에만 문의) */}
              {!isOwner && (
                <label className={`flex-1 flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${
                  targetType === 'OWNER'
                  ...
                  <input
                    ...
                    name="targetType"
                    value="OWNER"
                    checked={targetType === 'OWNER'}
                    onChange={() => setTargetType('OWNER')}
                    className="accent-amber-600"
                  />
                  <div>
                    <p className="text-sm font-bold text-slate-800">🏪 점주</p>
                    <p className="text-xs text-slate-500 mt-0.5">근무, 재고, 매장 내부 관련 문의</p>
                  </div>
                </label>
              )}
            </div>
