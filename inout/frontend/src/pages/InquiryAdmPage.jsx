...
import { getAdminInquiriesFromOwners, getAdminInquiriesFromEmployees } from '../api/inquiryApi';
...
const TABS = [
  { key: 'from-owners',    label: '🏪 가맹점주 문의',  desc: '점주가 본사로 보낸 문의 목록' },
  { key: 'from-employees', label: '👨‍🍳 매장직원 문의', desc: '매장 직원이 본사로 보낸 문의 목록 (내부 점주 문의는 표시되지 않음)' },
];
...
export default function InquiryAdmPage() {
  ...
  const [activeTab, setActiveTab] = useState('from-owners');
  ...
  const loadInquiries = useCallback((pg, tab) => {
    setLoading(true);
    const fn = tab === 'from-owners' ? getAdminInquiriesFromOwners : getAdminInquiriesFromEmployees;
    fn(pg, PAGE_SIZE)
      .then((data) => {
    ...
    <div className="space-y-5 max-w-7xl mx-auto">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">
          본사로 접수된 문의를 탭별로 확인합니다. 상세 페이지에서 AI 답변 초안을 생성할 수 있습니다.
        </p>
      </div>
        ...
        <table className="min-w-full divide-y divide-slate-200">
          ...
          <tbody className="divide-y divide-slate-100 bg-white">
            ...
            {inquiries.map((item, index) => {
              const validId = item.inquiryId || item.id;
              ...
              return (
                ...
                >
                  ...
                  <td className={`px-5 py-4 text-sm ${!item.isRead ? 'font-bold text-slate-900' : 'font-medium text-slate-700'}`}>
                    {item.title}
                  </td>
                  ...
                  <td className="px-5 py-4 text-sm text-slate-600"><PersonName name={item.authorName} /></td>
